...
export default function PostHeader({ author, post, hasBackButton = false, hasOptionsButton = true, user }: PostHeaderProps) {
    ...
    return (
        <CardHeader className={`${isPostPage ? 'px-0' : ''} z-10`}>
            <div className='flex justify-between items-center'>
                <div className='relative z-10 flex items-center gap-3'>
                    {hasBackButton && <PostBackButton />}
                    <Avatar className='cursor-pointer' onClick={() => router.push(`/profile/${author.username}`)}>
                    ...
                    <div className='text-zinc-800'>
                        <div className='flex gap-1 items-center'>
                            ...
                            <button onClick={() => router.push(`/profile/${user.username}`)} className='text-zinc-500 text-sm'>@{author.username}</button>
