...
export async function getUserProfile(username: string): Promise<User> {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch profile for "${username}": ${errorText}`,
      );
    }
    ...
    throw new Error(
      `Error fetching user profile for "${username}": ${detailedMessage}`,
    );
  }
}
...
export async function updateUserProfile(data: FormData) {
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/user/${user.id}?_method=PATCH`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
...
export async function getUserLikedPosts(username: string) {
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/posts/liked/${username}`,
      {
        next: {
          tags: ["posts"],
        },
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Failed to fetch liked posts for "${username}": ${errorText}`,
      );
    }
...
export async function storePost(data: Payload) {
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/posts/store`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export async function getFollowUsers(username: string) {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Failed to fetch followers for "${username}": ${errorText}`,
      );
    }
...
export async function followUser(username: string, id: number) {
  try {
    const { token } = await getAuthUser();
    ...
    const detailedMessage =
      error instanceof Error ? error.message : "Unknown error";
    console.error("Error in followUser:", detailedMessage);
    return { error: detailedMessage };
  }
}

export async function unfollowUser(username: string, id: number) {
  try {
    const { token } = await getAuthUser();
    ...
    const detailedMessage =
      error instanceof Error ? error.message : "Unknown error";
    console.error("Error in unfollowUser:", detailedMessage);
    return { error: detailedMessage };
  }
}
