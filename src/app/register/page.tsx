    ...
    return (
        <div className='h-screen bg-zinc-100 grid place-items-center'>
            <div className='grid grid-cols-2 max-w-5xl w-full bg-white rounded-2xl min-h-2/3 shadow-md gap-10'>
                <div className='bg-linear-to-br text-white from-blue-600 via-sky-500 to-lime-100 p-5 rounded-tl-2xl rounded-bl-2xl'>
                ...
                <div className='py-6 flex flex-col gap-5 justify-evenly'>
                    ...
                    <form onSubmit={submit} className='flex flex-col gap-5'>
                        <div className='bg-zinc-100 rounded-full px-4 py-5 w-11/12 flex gap-4 text-zinc-600'>
                            <User />
                            <input name='username' placeholder='Nome de usuário' type="text" className='w-full bg-zinc-100 focus:outline-hidden font-medium text-zinc-600' />
                        </div>
                        {errors?.errors?.username && <span className='text-red-500 px-4 text-sm'>{errors.errors.username.join(', ')}</span>}
                        <div className='bg-zinc-100 rounded-full px-4 py-5 w-11/12 flex gap-4 text-zinc-600'>
                            <Mail />
                            <input name='email' placeholder='E-mail' type="text" className='w-full bg-zinc-100 focus:outline-hidden font-medium text-zinc-600' />
                        </div>
                        {errors?.errors?.email && <span className='text-red-500 px-4 text-sm'>{errors.errors.email.join(', ')}</span>}
                        <div className='bg-zinc-100 rounded-full px-4 py-5 w-11/12 flex gap-4 text-zinc-600'>
                            <Lock />
                            <input name='password' placeholder='Senha' type="password" className='w-full bg-zinc-100 focus:outline-hidden font-medium text-zinc-600' />
                        </div>
